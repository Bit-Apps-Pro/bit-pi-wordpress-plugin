...
import { commandExistsSync, copyDir } from './build-helpers.mjs'
...
export default async function checkAndCopyFilesAndFolders() {
  const checks = filesAndFolders.map(async item => {
    try {
      await fs.promises.access(item)
      if (fs.lstatSync(item).isDirectory()) {
        copyDir(item, path.join(PLUGIN_SLUG, item))
      } else {
...
async function generateBuild() {
  ...
  fs.rmSync(`${PLUGIN_SLUG}/composer.lock`)
